using System;
using System.Text;
using Uyum.Net;
using Uyum.Objects;
using System.Runtime.Serialization;
using System.Runtime.InteropServices;


namespace WHM.Module.BusinessObjects
{
	[Guid("6f496e0f-f53e-4a2c-bb81-c2fc98240324")]
	public class EmpBreakTimetable : UyumObjectBase
	{

		public EmpBreakTimetable()
		{
		}

		private int _BreakTimetableId = 0;//integer
		private int _CoId = 0;//integer
		private int _BranchId = 0;//integer
		private int _EmployeeId = 0;//integer
		private DateTime _Date = DateTime.Now;//datetime
		private int _LocationType = 0;//integer
		private int _LocationId = 0;//integer
		private int _InputOutput = 0;//integer
		private int _InputTimetableId = 0;//integer
		private int _BreakReasonId = 0;//integer
		private int _CreateUserId = 0;//integer
		private DateTime _CreateDate = DateTime.Now;//timestamp
		private int _UpdateUserId = 0;//integer
		private DateTime _UpdateDate = DateTime.Now;//timestamp

		[Server]
		[DataInt32("emp_break_timetable_id", IsIdentity = true)]
		[UyumPrimaryKey(0)]
		public override int Id
		{
			get { return base.Id; }
			set { base.Id = value; }
		}

		[Server]
		[DataInt32("co_id")]
		public int CoId
		{
			get { return _CoId; }
			set { _CoId = value; }
		}

		[Server]
		[DataInt32("branch_id")]
		public int BranchId
		{
			get { return _BranchId; }
			set { _BranchId = value; }
		}

		[Server]
		[DataInt32("employee_id")]
		public int EmployeeId
		{
			get { return _EmployeeId; }
			set { _EmployeeId = value; }
		}

		[Server]
		[DataTime("date")]
		public DateTime Date
		{
			get { return _Date; }
			set { _Date = value; }
		}

		[Server]
		[DataInt32("location_type")]
		public int LocationType
		{
			get { return _LocationType; }
			set { _LocationType = value; }
		}

		[Server]
		[DataInt32("location_id")]
		public int LocationId
		{
			get { return _LocationId; }
			set { _LocationId = value; }
		}

		[Server]
		[DataInt32("input_output")]
		public int InputOutput
		{
			get { return _InputOutput; }
			set { _InputOutput = value; }
		}

		[Server]
		[DataInt32("input_timetable_id")]
		public int InputTimetableId
		{
			get { return _InputTimetableId; }
			set { _InputTimetableId = value; }
		}

		[Server]
		[DataInt32("break_reason_id")]
		public int BreakReasonId
		{
			get { return _BreakReasonId; }
			set { _BreakReasonId = value; }
		}

		[Server]
		[DataInt32("create_user_id")]
		public int CreateUserId
		{
			get { return _CreateUserId; }
			set { _CreateUserId = value; }
		}

		[Server]
		[DataTime("create_date")]
		public DateTime CreateDate
		{
			get { return _CreateDate; }
			set { _CreateDate = value; }
		}

		[Server]
		[DataInt32("update_user_id")]
		public int UpdateUserId
		{
			get { return _UpdateUserId; }
			set { _UpdateUserId = value; }
		}

		[Server]
		[DataTime("update_date")]
		public DateTime UpdateDate
		{
			get { return _UpdateDate; }
			set { _UpdateDate = value; }
		}


	}
	[DataContract(Name = "EmpBreakTimetable")]
	public class EmpBreakTimetableInfo
	{

		public EmpBreakTimetableInfo()
		{
		}

		[DataMember(Name = "emp_break_timetable_id")]
		public int Id { get; set; }

		[DataMember(Name = "co_id")]
		public int CoId { get; set; }

		[DataMember(Name = "branch_id")]
		public int BranchId { get; set; }

		[DataMember(Name = "employee_id")]
		public int EmployeeId { get; set; }

		[DataMember(Name = "date")]
		public DateTime Date { get; set; }

		[DataMember(Name = "location_type")]
		public int LocationType { get; set; }

		[DataMember(Name = "location_id")]
		public int LocationId { get; set; }

		[DataMember(Name = "input_output")]
		public int InputOutput { get; set; }

		[DataMember(Name = "input_timetable_id")]
		public int InputTimetableId { get; set; }

		[DataMember(Name = "break_reason_id")]
		public int BreakReasonId { get; set; }

		[DataMember(Name = "create_user_id")]
		public int CreateUserId { get; set; }

		[DataMember(Name = "create_date")]
		public DateTime CreateDate { get; set; }

		[DataMember(Name = "update_user_id")]
		public int UpdateUserId { get; set; }

		[DataMember(Name = "update_date")]
		public DateTime UpdateDate { get; set; }

	}
}
