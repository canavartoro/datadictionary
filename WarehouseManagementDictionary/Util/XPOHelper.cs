...
namespace WarehouseManagementDictionary
{
    static class XPOHelper
    {
        public static void XPOStart()
        {
            try
            {
                string conString = SQLiteConnectionProvider.GetConnectionString(string.Format("{0}\\{1}_dictionary.dat", System.Windows.Forms.Application.StartupPath, DateTime.Now.Year));
